...
void putLiteralToTrueInModel(int lit) {
    if (lit > 0) {
        model[lit] = TRUE;
    } else {
        model[-lit] = FALSE;
...
void insertIntoModelStack(vector<pair<int, int> >& insertionOrder) {
    int n = insertionOrder.size();
    for (int i = 0; i < n; ++i) {
        modelStack.push_back(insertionOrder[i].first);
    }
}

void clearPropagationQueue (vector<pair<int, int> >& insertionOrder){
    int n = insertionOrder.size();
    for (int i = 0; i < n; ++i) {
        int lit = insertionOrder[i].first;
...
bool propagateGivesConflict() {
    while (indexOfNextLitToPropagate < modelStack.size()) {
        ...
        vector<pair<int, int> > insertionOrder;
        for (uint i = 0; i < len; ++i) {
            int clauseToSearch = (*clausesOnLit)[i];
            if (isClausePropagable(clauseToSearch)) {
                for (uint k = 0; k < 3; ++k) {
                    int lit = clauses[clauseToSearch][k];
                    int val = currentValueInModel(lit);
                    if (val == UNDEF) {
                        insertionOrder.emplace_back(lit, getLiteralImportance(lit));
                        putLiteralToTrueInModel(lit);
                        ++numberPropagations;
                        break;
                    }
                }
                ...
                nConflicts += 3;
                clearPropagationQueue(insertionOrder);
                return true; // conflict! all lits false
            }
        }
        sort(insertionOrder.begin(), insertionOrder.end(), comp);
        insertIntoModelStack(insertionOrder);
    }
    return false;
}
