...
tsp( Cities, AccumulatedKm, [ CurrentCity | RouteSoFar ] ):-
    orderCities(CurrentCity,Cities,Order),
    select( City, Order, RemainingCities ),  % select next city to visit
...
%    nearCity(CurrentCity,City,Cities,RemainingCities).
%myselect(CurrentCity, City, Cities, RemainingCities):-
%    nearCity(CurrentCity,City1,Cities,Remaining),
%    myselect(CurrentCity,City,Remaining,Remaining2),
%    append([City1],Remaining2,RemainingCities).

lowerBoundOfRemainingCities([],0):- !.

lowerBoundOfRemainingCities([C|LeftCities],Km):- minDist(C,Dist),
    lowerBoundOfRemainingCities(LeftCities,Km2), Km is Km2 + Dist.

%nearCity(_,C,[C],[]):- !.
%nearCity(CurrentCity, City1, [First|Cities], RemainingCities):-
%    distance(CurrentCity,First,D1), nearCity(CurrentCity,City1,Cities,Remain),
%    distance(CurrentCity,City1,D2), D2 =< D1, append(Remain,[First],RemainingCities).
%nearCity(CurrentCity, First, [First|Cities], Cities):-
%    distance(CurrentCity,First,D1), nearCity(CurrentCity,City1,Cities,_),
%    distance(CurrentCity,City1,D2), D2 > D1.

%minDist(C,RemainingCities,KM):- findall(Distances,(member(B,RemainingCities), distance(C,B,Distances)),LDistances),
%  min_member(KM,LDistances).

%leggo:- findall(MinDistances,(between(1,22,A),minDist(A,MinDistances)),List), write(List).

orderCities(CurrentCity,Cities,Order):-
    findall([D,C], (member(C,Cities),distance(CurrentCity,C,D)),L),
    sort(L,L1),
    findall(C,member([_,C],L1),Order), !.

minDist(A,Km):-
  M=[20,18,42,31,17,38,19,55,36,33,26,17,17,18,35,34,28,35,28,33,19,20],
    nth1(A,M,Km), !.
