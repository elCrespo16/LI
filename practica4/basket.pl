...
eachRoundTvMatch:- round(R), findall(match(S,T,R),(team(S), team(T), tvMatch(S,T)),LitsA), 
        findall(match(T,S,R),(team(S), team(T), tvMatch(S,T)),LitsB), append(LitsA,LitsB,Lits), atLeast(1,Lits), fail.
eachRoundTvMatch.

noDoublesOnXRounds:- noDoubles(Rounds), member(R,Rounds), team(T), writeClause([-double(T,R)]), fail.
noDoublesOnXRounds.

homeFull:- team(T), notHome(T,Rounds), member(R,Rounds), writeClause([-home(T,R)]), fail.
homeFull.
...
oneVsOneOnce:- team(A), team(B), difTeams(A,B), findall(match(A,B,R), round(R), LitsA), findall(match(B,A,R), round(R), LitsB),
            append(LitsA,LitsB,Lits), exactly(1,Lits), fail.
oneVsOneOnce.

implicationsHome:- team(T), team(D), difTeams(T,D), round(R), writeClause([-match(T,D,R),home(T,R)]), fail.
implicationsHome.

implicationsDouble:- team(T), round(R), R1 is R - 1, round(R1), writeClause([-home(T,R),-home(T,R1),double(T,R)]),
