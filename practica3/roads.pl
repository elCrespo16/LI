...
writeClauses:- 
	maxStationsGenerated,
	stationXroad,
	true,!.                    % this way you can comment out ANY previous line of writeClauses
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

maxStationsGenerated:- findall(install(C),city(C),Lits), maxStations(N), atMost(N,Lits).

stationXroad:- road(A,B), city(A), city(B), Lits = [install(A), install(B)], atLeast(1,Lits), fail.
stationXroad.
