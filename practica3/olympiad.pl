...
satVariable( chosen(S) ):- student(S).
...
exactlyKstudents:- findall(chosen(S),student(S),Lits), numMembersTeam(T), exactly(T,Lits).

areWefriends:- student(S), student(T), S \= T, fakeFriends(S,T), fail.
areWefriends.
fakeFriends(E1,E2):- not(friends(E1,E2)), friends(E2,E1), writeClause([-chosen(E1),-chosen(E2)]), !.
fakeFriends(E1,E2):- not(friends(E1,E2)), friends(E1,E2), writeClause([-chosen(E1),-chosen(E2)]), !.
fakeFriends(E1,E2):- not(friends(E1,E2)), not(friends(E1,E2)), E1 < E2, writeClause([-chosen(E1),-chosen(E2)]), !.
fakeFriends(_,_):- !.
experts:- student(S), expertise(S,E), findall(chosen(T),(expertise(T,E),S < T),Lits), Lits \= [], atMost(1,[chosen(S)|Lits]), fail.
experts.
